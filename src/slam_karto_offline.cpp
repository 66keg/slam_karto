...
bool g_flag_finish;
std::string g_odom_frame, g_base_frame;
...
class SlamKarto
{
  public:
    ...
    bool publishMapService(std_srvs::Empty::Request  &req,
  ...
  private:
    ...
    ros::ServiceServer map_pub_service_;
    bool flag_publish_map_;
...
SlamKarto::SlamKarto() :
        ...
        marker_count_(0), flag_publish_map_(false)
{
  ...
  map_pub_service_ = node_.advertiseService("publish_map", &SlamKarto::publishMapService, this);
  ...
  g_odom_frame = odom_frame_;
  g_base_frame = base_frame_;
  ...
  if(!private_nh_.getParam("laser_range_threshold", laser_range_threshold_))
    laser_range_threshold_ = 80.0;
...
bool
SlamKarto::publishMapService(std_srvs::Empty::Request  &req,
                             std_srvs::Empty::Response &res)
{
  flag_publish_map_ = true;
  return true;
}
...
SlamKarto::getLaser(const sensor_msgs::LaserScan::ConstPtr& scan)
{
  ...
  if(lasers_.find(scan->header.frame_id) == lasers_.end())
  {
    ...
    laser->SetRangeThreshold(laser_range_threshold_);
...
{
  ros::init(argc, argv, "slam_karto_offline");
  ros::NodeHandle n;

  SlamKarto kn;

  ROS_INFO("karto slam offline started");
  ...
  ros::Publisher pub_odom = n.advertise<nav_msgs::Odometry>(odom_topic.c_str(), 1);
  ros::Publisher pub_scan = n.advertise<sensor_msgs::LaserScan>(scan_topic.c_str(), 1);
  ...
  ros::param::get("~bag_filename", bag_filename);
  if(bag_filename.empty())
  {
    ROS_ERROR("bag file name is not set");
    ros::shutdown();
    return 1;
  }
  else
  {
    ROS_INFO("bag file load from: %s", bag_filename.c_str());
  ...
  sensor_msgs::LaserScan::ConstPtr current_scan;
  geometry_msgs::TransformStamped current_tf;

  g_flag_finish = false;

  double param_start_time_sec = 0;
  ros::param::get("~start_time_sec", param_start_time_sec);
  bool flag_passed_start_time = false;

  bool flag_time_initialized = false;
  ros::Time time_bag_initial, time_bag_current;

  BOOST_FOREACH(rosbag::MessageInstance const m, view)
  {
    if (m.getTopic() == scan_topic)
    {
      sensor_msgs::LaserScan::ConstPtr d_info_ptr = m.instantiate<sensor_msgs::LaserScan>();
      current_scan = d_info_ptr;

      if(!flag_time_initialized)
      {
        time_bag_initial = current_scan->header.stamp;
        flag_time_initialized = true;
      }
      time_bag_current = current_scan->header.stamp;

      double time_bag_from_initial = (time_bag_current - time_bag_initial).toSec();
      ROS_INFO("%f sec", time_bag_from_initial);

      if (d_info_ptr != NULL && time_bag_from_initial > param_start_time_sec)
      {
        flag_passed_start_time = true;
        pub_scan.publish(*current_scan);
        kn.laserCallback(current_scan);
      }
    }

    if (m.getTopic() == tf_topic)
    {
      tf::tfMessage::ConstPtr d_info_ptr = m.instantiate<tf::tfMessage>();
      int size = d_info_ptr->transforms.size();
      for(int i=0; i<size; i++)
      {
        if(d_info_ptr->transforms[i].header.frame_id == g_odom_frame && d_info_ptr->transforms[i].child_frame_id == g_base_frame)
        {
          current_tf = d_info_ptr->transforms[i];
          br.sendTransform(current_tf);
          break;
        }
      }
    }

    if(flag_passed_start_time)
    {
      if (m.getTopic() == odom_topic)
      {
        nav_msgs::Odometry::ConstPtr d_info_ptr = m.instantiate<nav_msgs::Odometry>();
        if (d_info_ptr != NULL)
        {
          pub_odom.publish(*d_info_ptr);
        }
      }
  ...
  double compute_time = (end_time - start_time).toSec();
  ROS_INFO("Mapping is finished at %f sec", compute_time);
